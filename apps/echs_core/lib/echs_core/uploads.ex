defmodule EchsCore.Uploads do
  ...
  def store_file(path, opts \\ []) when is_binary(path) do
    ...
    with {:ok, %File.Stat{type: :regular, size: size}} <- File.stat(path),
         ...
         {:ok, mime} <- image_mime(filename, content_type),
         {:ok, upload_id} <- copy_and_write_meta(path, filename, mime) do
      {:ok,
       %{
         upload_id: upload_id,
         kind: "image",
  ...
  defp copy_and_write_meta(path, filename, mime) do
    dir = upload_dir()
    :ok = File.mkdir_p(dir)

    upload_id = generate_upload_id()
    stored_filename = upload_id <> preferred_ext(filename, mime)

    dest_path = Path.join(dir, stored_filename)
    meta_path = Path.join(dir, upload_id <> ".json")

    with :ok <- File.cp(path, dest_path),
         :ok <-
  ...
  defp read_meta(upload_id) do
    meta_path = Path.join(upload_dir(), upload_id <> ".json")
  ...
  defp image_mime(filename, content_type) do
